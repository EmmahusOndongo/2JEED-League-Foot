...
public class GameServices {
    ...
    public EventDto addEventToGame(UUID gameId, EventDto eventDto) {
        ...
        if (optionalGame.isPresent()) {
            ...
            eventEntity.setType(eventDto.type());
            eventEntity.setPlayer(eventDto.player());
            eventEntity.setMinute(eventDto.minute());
            eventEntity.setGame(game);

            eventEntity = eventRepository.save(eventEntity);
            game.getEvents().add(eventEntity);
            gameRepository.save(game);

            return GameMapper.toEventDto(eventEntity);
        } else {
            throw new RuntimeException("Game not found with ID: " + gameId);
        }
    }
